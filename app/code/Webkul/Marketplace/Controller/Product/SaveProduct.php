...
class SaveProduct
{
    ...
    public function saveProductData($sellerId, $wholedata)
    {
        ...
        if (!empty($wholedata['id'])) {
            ...
            $wholedata = $this->adminStoreMediaImages($mageProductId, $wholedata, $storeId);
        } else {
            $mageProductId = '';
            $editFlag = 0;
            $storeId = 0;
        ...
        if ($mageProductId) {
            $mpProdStatus = $helper->getIsProductEditApproval() ?
            ...
            if ($helper->getIsProductApproval() && !$helper->getIsProductEditApproval()) {
                ...
                foreach ($sellerProductColls as $sellerProductColl) {
                    $mpProdStatus = !$sellerProductColl->getIsApproved() ?
                    SellerProduct::STATUS_DISABLED : SellerProduct::STATUS_ENABLED;
                }
            }
        } else {
            $mpProdStatus = $helper->getIsProductApproval() ?
            SellerProduct::STATUS_PENDING : SellerProduct::STATUS_ENABLED;
            $status = $helper->getIsProductApproval() ?
            SellerProduct::STATUS_DISABLED : SellerProduct::STATUS_ENABLED;
        }

        $status = isset($wholedata['status'])?$wholedata['status']:$status;
        ...
        */
        ...
        if (isset($set) && isset($type)) {
            ...
            if (!in_array($type, $allowedtypes) || !in_array($set, $allowedsets)) {
                $returnArr['error'] = 1;
                $this->_messageManager->addWarning(__('Product Type Invalid Or Not Allowed'));
                return $returnArr;
            }
        } else {
            $returnArr['error'] = 1;
        ...
        if ($type == SellerProduct::PRODUCT_TYPE_CONFIGURABLE) {
            $allowedtypes = explode(',', $helper->getAllowedProductType());
            $isWeightEmpty =  empty($wholedata['product']['weight']);
            if (!in_array(SellerProduct::PRODUCT_TYPE_VIRTUAL, $allowedtypes) && $isWeightEmpty) {
                $returnArr['error'] = 1;
    ...
    private function saveMaketplaceProductTable(
        ...
        if ($mageProductId) {
            ...
            $mpProduct = $this->_mpProductFactory->create()->load($sellerProductId);
            $mpProduct->setMageproductId($mageProductId);
            $mpProduct->setSellerId($sellerId);
            $mpProduct->setStatus($status);
            $isApproved = 1;
            if ($helper->getIsProductEditApproval()) {
                $mpProduct->setAdminPendingNotification(2);
            }
            if (!$editFlag) {
                $mpProduct->setCreatedAt($this->_date->gmtDate());
                $mpProduct->setAdminPendingNotification(1);
        ...
        foreach ($associatedProductIds as $associatedProductId) {
            if ($associatedProductId) {
                ...
                $mpProductAssoc = $this->_mpProductFactory->create()->load($sellerAssociatedProductId);
                $mpProductAssoc->setMageproductId($associatedProductId);
                $mpProductAssoc->setStatus($status);
                if (!$editFlag) {
                    /* If new product is added*/
                    $mpProductAssoc->setCreatedAt($this->_date->gmtDate());
                }
                if ($editFlag) {
                    $mpProductAssoc->setAdminPendingNotification(2);
                }
                $mpProductAssoc->setSellerId($sellerId);
                $mpProductAssoc->setIsApproved($isApproved);
                $mpProductAssoc->save();
            }
        }
    }
    ...
    private function manageSellerProductStock($requestProductData)
    {
        if ($requestProductData) {
            ...
            if ($stockData['use_config_manage_stock'] == 1 && !isset($stockData['manage_stock'])) {
                $stockData['manage_stock'] = $this->_stockConfiguration
                ->getManageStock();
            }
    ...
    private function sendProductMail($data, $sellerId, $editFlag = null)
    {
        ...
        $emailTempVariables['productName'] = $data['product']['name'];
        $emailTempVariables['categoryName'] = $categoryname;
        $emailTempVariables['adminName'] = $adminUsername;
        if ($editFlag == null) {
            $emailTempVariables['message'] = __(
