...
class History extends \Magento\Framework\View\Element\Template
{
    ...
    public function getAllSalesOrder()
    {
        ...
        try {
            if (!$this->salesOrderLists) {
                ...
                $mpOrderTable = $this->_orderCollectionFactory->create()->getTable("marketplace_orders");
                if (isset($paramData['s'])) {
                    $filterOrderid = $paramData['s'];
                }
                if (isset($paramData['orderstatus'])) {
                    $filterOrderstatus = $paramData['orderstatus'];
                }
                ...
                if (!empty($filterDataTo) && !empty($filterDataFrom)) {
                ...
                $collection->getSelect()->joinLeft(
                    $mpOrderTable . ' as mo',
                    'main_table.order_id = mo.order_id',
                    []
                );
                if ($paramData["type"] != \Webkul\Marketplace\Model\Orders::FILTER_ALL) {
                    $collection->addFieldToFilter("mo.order_status", $paramData["type"]);
                }
     ...
     *
     * @param string $mpOrderStatus
     * @return string
     */
    public function getOrderStatusLabel($mpOrderStatus)
    {
        return $this->ordersHelper->getOrderStatusLabel($mpOrderStatus);
    }
}
