        ...
        try {
            ...
            if (!empty($paramData['is_seller']) && !empty($paramData['profileurl']) && $paramData['is_seller'] == 1) {
                ...
                if (!$profileurlcount->getSize()) {
                    ...
                    $model->save();
                    if ($this->mpHelper->getCurrentStoreId() !== 0) {
                        $model = $this->mpSellerFactory->create();
                        $model->setData('is_seller', $status);
                        $model->setData('shop_url', $paramData['profileurl']);
                        $model->setData('seller_id', $customerid);
                        $model->setData('store_id', $this->mpHelper->getCurrentStoreId());
                        $model->setCreatedAt($this->_date->gmtDate());
                        $model->setAdminNotification(0);
                        $model->save();
                    }
                    ...
                    if ($partnerApprovalStatus) {
                        ...
                        $emailTemplateVariables['sellerName'] = $customer->getFirstName().' '.
                        $customer->getLastName();
                        $emailTemplateVariables['sellerUrl'] = $this->urlBackendModel->getUrl(
                            'customer/index/edit',
                            ['id' => $customer->getId()]
                        );
                        $emailTemplateVariables['adminName'] = $helper->getAdminName();
