...
define([
    ...
    $.widget('mage.sellerAddProduct', {
        ...
        _create: function () {
            ...
            $('#wk-mp-save-draft-product-btn').click(function () {
                if ($('#edit-product').valid()) {
                    let draftId = $('#wk-mp-save-draft-product-btn').data("draft_id");
                    $("#edit-product").append('<input type="hidden" name="draft" value="'+(draftId)+'">');
                }
                $('#save-btn').trigger('click');
            });
