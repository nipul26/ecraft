...
class Attributes extends \Magento\Ui\DataProvider\AbstractDataProvider
{
    ...
    */
    public function getData()
    {
        ...
        $items = [];
        $skippedItems = 0;
        $sellerAttributeName = HelperData::SELLER_ID_ATTRIBUTE_NAME;
        foreach ($this->getCollection()->getItems() as $attribute) {
            if ($this->configurableAttributeHandler->isAttributeApplicable($attribute)
             && !empty($attribute->getVisibleToSeller())) {
                ...
                if ($attribute->getAttributeCode() == $sellerAttributeName) {
                    $skippedItems++;
                } elseif (in_array($attribute->getId(), $isLoginVendorAttribute)) {
