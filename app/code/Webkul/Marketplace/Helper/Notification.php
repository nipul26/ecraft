...
class Notification extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getAllNotificationIds($sellerId)
    {
        try {
            ...
            $collectionProductNotif = $this->collectionFactory->create()
            ->addFieldToFilter(
                'seller_id',
                $sellerId
            );
            $collectionProductNotif->getSelect()->join(
            ...
            $idsProductNotif = $collectionProductNotif->getAllIds();
            // get all notification order ids
            $collectionOrderNotif = $this->collectionFactory->create()
            ->addFieldToFilter(
                'seller_id',
                $sellerId
            );
            $collectionOrderNotif->getSelect()->join(
            ...
            $idsOrderNotif = $collectionOrderNotif->getAllIds();
            // get all notification transaction ids
            $collectionTransNotif = $this->collectionFactory->create()
            ->addFieldToFilter(
                'seller_id',
                $sellerId
            );
            $collectionTransNotif->getSelect()->join(
            ...
            $idsTransNotif = $collectionTransNotif->getAllIds();
            // get all notification review ids
            $collectionReviewNotif = $this->collectionFactory->create()
            ->addFieldToFilter(
                'seller_id',
                $sellerId
            );
            $collectionReviewNotif->getSelect()->join(
            ...
            $idsReviewNotif = $collectionReviewNotif->getAllIds();

            $ids = array_merge($idsProductNotif, $idsOrderNotif, $idsTransNotif, $idsReviewNotif);
        } catch (\Exception $e) {
            $ids = [];
     ...
     *
     * @param \Webkul\Marketplace\Model\Feedback $modelData
     * @param string $type
     * @return void
     */
     ...
     *
     * @param \Webkul\Marketplace\Model\Notification $rowData
     * @param string $url
     * @return string
     */
