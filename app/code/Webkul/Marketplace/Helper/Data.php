...
use Webkul\Marketplace\Model\ResourceModel\DraftProduct\CollectionFactory as DraftProductColl;
...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public const MARKETPLACE_GENERAL_SETTINGS_FULFILMENT_LABEL = "marketplace/general_settings/fulfilment_label";

    public const MARKETPLACE_GENERAL_SETTINGS_FULFILMENT_IMAGE = "marketplace/general_settings/fulfilment_image";

    public const MARKETPLACE_GENERAL_SETTINGS_FULFILMENT_TEXT = "marketplace/general_settings/fulfilment_text";

    public const FULFILLED_BY_MARKETPLACE = 1;
    /**
    ...
    /**
     * @var DraftProductColl
     */
    protected $draftProductColl;
    /**
     ...
     * @param DraftProductColl $draftProductColl
     * @param \Magento\Framework\Stdlib\DateTime\TimezoneInterface $timezone
     * @param \Magento\Shipping\Model\CarrierFactory $carrierFactory
     * @param \Magento\Tax\Model\ClassModel $classModel
     */
    public function __construct(
        ...
        DraftProductColl $draftProductColl,
        \Magento\Framework\Stdlib\DateTime\TimezoneInterface $timezone,
        \Magento\Shipping\Model\CarrierFactory $carrierFactory,
        \Magento\Tax\Model\ClassModelFactory $classModel
    ) {
        ...
        $this->draftProductColl = $draftProductColl;
    ...
    {
        return $this->classModel->create()->getCollection()->addFieldToFilter('class_type', 'PRODUCT');
    }
    ...
    {
        $data = [];
        $bannerpic = $logopic = $countrylogopic = $fulfilmentImage = '';
        $isDefaultBanner = $isDefaultLogo = $isDefaultFulfilment = 0;
        $sellerId = $this->getCustomerId();
        $model = $this->getSellerCollectionObj($sellerId);
        ...
        foreach ($model as $value) {
            ...
            $fulfilmentImage = $value->getFulfilmentImage() ? $value->getFulfilmentImage() : '';
            ...
            if (strlen($logopic) <= 0) {
                $logopic = 'default/noimage.png';
                $isDefaultLogo = 1;
            }
            if (strlen($fulfilmentImage) <= 0) {
                $fulfilmentImage = 'fulfilmentImage.png';
                $isDefaultFulfilment = 1;
            }
        ...
        $data['fulfilment_image'] = $fulfilmentImage;
        $data['is_default_fulfilment'] = $isDefaultFulfilment;
    ...
    /**
     * Check if seller registration will show at customer register page
     *
     * @return string
     */
    public function getSellerBlockAtRegistration()
    {
        return  $this->scopeConfig->getValue(
            'marketplace/landingpage_settings/allow_seller_registration_block',
            ScopeInterface::SCOPE_STORE
        );
    }
    ...
    public function getSellerRegistrationUrl()
    {
        ...
        if (!(int)$this->getSellerBlockAtRegistration()) {
            $url = $this->_urlBuilder->getUrl(
    ...
    {
        $status = "processing";
        $qtyToShip = $qtyToRefund = $qtyToInvoice = 0;
        ...
        foreach ($order->getAllVisibleItems() as $item) {
            ...
            if ($itemSellerId == $sellerId) {
                ...
                $qtyToInvoice += $this->getQtyToInvoiceCount($item);
                $qtyToRefund += $this->getQtyToRefundCount($item);
            }
        }
        if ($qtyToShip == 0 && $qtyToInvoice == 0) {
            $status = "complete";
        }
     ...
     * @return int|float
     */
    public function getQtyToInvoiceCount($item)
    {
        $remaingQtyToInvoice =  $item->getQtyOrdered() -
            $item->getQtyInvoiced() -
            $item->getQtyCanceled();
        if ($remaingQtyToInvoice) {
            return $remaingQtyToInvoice;
        }
        return 0;
    }
    ...
    public function getQtyToShipCount($item)
    {
        if ($item->getIsVirtual()) {
            $remaingQtyToShip = ($item->getQtyInvoiced() == 0) ? 1 : $item->getQtyOrdered() -
                $item->getQtyInvoiced() -
                $item->getQtyCanceled();
        } else {
     ...
     *
     * @param int $entityId
     * @return DraftProductColl
     */
    public function getDraftProductCollection($entityId = 0)
    {
        $draftProdColl = $this->draftProductColl->create();
        if ($entityId) {
            $draftProdColl->addFieldToFilter('entity_id', $entityId);
            return $draftProdColl->getFirstItem();
        }
        return $draftProdColl;
    }
    /**
     * Change date format
     *
     ...
     * @return string
     */
    public function changeDateFormat($date, $format = 'd F y H:i:s')
    {
        return $this->timezone->date($date)->format($format);
    }

    /**
     * Get Mp Product listing actions for Webkul\Marketplace\Plugin\Elasticsearch7\SearchAdapter object
     *
     * @return array
     */
    public function getAllowedActionsForAdapter()
    {
        return [
